...
for i in range(len(fe_s)):
    while r_planet <= 1000e3:
        print(j)
        # in the planet
        mol_fe = 1000 * fe_s[i] * 0.01 * ocean_mass(r_planet, r_planet - planet_core_radius) / (molar_mass_fe)
        mol_ni = 1000 * ni_s * 0.01 * ocean_mass(r_planet, r_planet - planet_core_radius) / (molar_mass_ni)
        mol_si = 1000 * si_s[i] * 0.01 * ocean_mass(r_planet, r_planet - planet_core_radius) / (molar_mass_si)
        mol_mg = 1000 * mg_s[i] * 0.01 * ocean_mass(r_planet, r_planet - planet_core_radius) / (molar_mass_mg)
        mol_v = 1000 * v_s * 0.01 * ocean_mass(r_planet, r_planet - planet_core_radius) / (molar_mass_v)
        mol_o = 1000 * o_s[i] * 0.01 * ocean_mass(r_planet, r_planet - planet_core_radius) / (molar_mass_o)
        ...
        delivered_fe_s = fe_s[i] * 0.01 * ocean_mass(r_impactor, r_impactor - c_impactor) * 1000 / molar_mass_fe
        delivered_si = 1000 * si_s[i] * 0.01 * ocean_mass(r_impactor, r_impactor - c_impactor) / molar_mass_si
        delivered_mg = 1000 * mg_s[i] * 0.01 * ocean_mass(r_impactor, r_impactor - c_impactor) / molar_mass_mg
        delivered_v = 1000 * v_s * 0.01 * ocean_mass(r_impactor, r_impactor - c_impactor) / molar_mass_v
        delivered_o = 1000 * o_s[i] * 0.01 * ocean_mass(r_impactor, r_impactor - c_impactor) / molar_mass_o
        ...
        j += 1
    save_data(X_Fe, X_Si, X_Ni, X_Va, X_FeO, X_SiO2, X_NiO, X_Mg, X_VO, gravity, pressure, temperature, planet_size, impactor_size, mantle_depth, fO2, "./data/deep-MO/heterogeneous_accretion_{}wt%_FeO.txt".format(fe_s[i]))
